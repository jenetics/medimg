            ...
            for ( i = 0; i < 8; i++) {
                    for ( j = 0; j < 6; j++) {
                            ...
                            if (Math.abs(tmp1 - normal[j][3]) < EPSILON) {
                                    ...
                                    surfacePoints[j].addElement(new Point3D(end));
                            }
                    }
            }
            //Volume
                    ...
                    if (surfacePoints[i].size() > 2) {
                            ...
                            for (j = surfacePoints[i].size() - 2; j >= 0; j--) {
                                    for ( k = 1; k <= j; k++) {
                                            ...
                                            for ( l = 0; l < 3; l++) {
                                                    matDet[0][l] = ((Point3D)surfacePoints[i].elementAt(0)).getValue(l);
                                            }
                                            matDet[0][3] = 1.0;
                                            // 2. point = starting point + normalvector
                                            for ( l = 0; l < 3; l++) {
                                                    matDet[1][l] = ((Point3D)surfacePoints[i].elementAt(0)).getValue(l) + vertices[i][l];
                                            }
                                            matDet[1][3] = 1.0;
                                            // 3. point
                                            for ( l = 0; l < 3; l++) {
                                                    matDet[2][l] = ((Point3D)surfacePoints[i].elementAt(field[k])).getValue(l);
                                            }
                                            matDet[2][3] = 1.0;
                                            // point to check
                                            for ( l = 0; l < 3; l++) {
                                                    matDet[3][l] = ((Point3D)surfacePoints[i].elementAt(field[k + 1])).getValue(l);
                                            }
                                            matDet[3][3] = 1.0;
                            ...
                            for ( j = 0; j < surfacePoints[i].size() - 2; j++) {
                                    for ( l = 0; l < 3; l++) {
                                                    triangle[0][l] = ((Point3D)surfacePoints[i].elementAt(surfacePoints[i].size() - 1)).getValue(l);
                                    }
                                    for ( l = 0; l < 3; l++) {
                                                    triangle[1][l] = ((Point3D)surfacePoints[i].elementAt(j)).getValue(l);
                                    }
                                    for ( l = 0; l < 3; l++) {
                                                    triangle[2][l] = ((Point3D)surfacePoints[i].elementAt(j + 1)).getValue(l);
                                    }
                                    // add to volume
    ...
    public double getIntersectingVolume(BoundingBox bb) {
                            ...
                            if (ADJ[i][j]) {
                                    ...
                                    for  (k = 0; k < 6; k++) {
                                            ...
                                            if (tmp2 != 0) {
                                                    ...
                                                    intersectingPoints.addElement(new Point3D(erg));
                                            }
                                    }
                            }
                    }
            ...
            for ( i = 0; i < intersectingPoints.size(); i++) {
                    for ( j = i + 1; j < intersectingPoints.size(); j++) {
                            if ((Math.abs(((Point3D)intersectingPoints.elementAt(i)).getValue(0) - ((Point3D)intersectingPoints.elementAt(j)).getValue(0)) < EPSILON) && 
                                    (Math.abs(((Point3D)intersectingPoints.elementAt(i)).getValue(1) - ((Point3D)intersectingPoints.elementAt(j)).getValue(1)) < EPSILON) && 
                                    (Math.abs(((Point3D)intersectingPoints.elementAt(i)).getValue(2) - ((Point3D)intersectingPoints.elementAt(j)).getValue(2)) < EPSILON)) {
                                    intersectingPoints.remove(j);
                                    /*end[0] = 0; end[1] = 0; end[2] = 0;
                                    end = (Point3D)intersectingPoints.elementAt(j);
            ...
            for ( i = 0; i < intersectingPoints.size(); i++) {
                    ...
                    for (j = 0; j < 6; j++) {
                            ...
                            tmp1 = normal[j][0] * ((Point3D)intersectingPoints.elementAt(i)).getValue(0) + 
                                       normal[j][1] * ((Point3D)intersectingPoints.elementAt(i)).getValue(1) + 
                                       normal[j][2] * ((Point3D)intersectingPoints.elementAt(i)).getValue(2);
            ...
            for ( i = 0; i < intersectingPoints.size(); i++) {
                    ...
                    for (j = 0; j < 6; j++) {
                            // tmp1 = vec(n) * vec(point) 
                            tmp1 = srcPlanes[j][0] * ((Point3D)intersectingPoints.elementAt(i)).getValue(0) + 
                                       srcPlanes[j][1] * ((Point3D)intersectingPoints.elementAt(i)).getValue(1) + 
                                       srcPlanes[j][2] * ((Point3D)intersectingPoints.elementAt(i)).getValue(2);
                            // tmp2 = vec(n) * vec(point_on_plain)
            ...
            for ( i = 0; i < 6; i++) {
                    ...
                    if (control) {
                            for ( k = 0; k < allplanes.size(); k++) {
                                    for ( j = 0; j < 4 ; j++) {
                                            if (Math.abs(normal[i][j] - ((Point3D)allplanes.elementAt(k)).getValue(j)) > EPSILON) {
                                                    control = true;
                                                    break;
                                            }
                                            control = false;
                            ...
                            if (control) {
                                    ...
                                    plane[2] = normal[i][2];
                                    plane[3] = normal[i][3];
                                    allplanes.addElement(new Point3D(plane));
                            }
                    }
            }
            // generate all plains
            ...
            for ( i = 0; i < 6; i++) {
                    ...
                    if (control) {
                            for ( k = 0; k < allplanes.size(); k++) {
                                    for ( j = 0; j < 4 ; j++) {
                                            if (Math.abs(srcPlanes[i][j] - ((Point3D)allplanes.elementAt(k)).getValue(j)) > EPSILON) {
                                                    control = true;
                                                    break;
                                            }
                                            control = false;
            ...
            for ( i = 0; i < intersectingPoints.size(); i++) {
                    ...
                    for ( j = 0; j < allplanes.size(); j++) {
                            // tmp1 = vec(n) * vec(point) 
                            tmp1 = ((Point3D)allplanes.elementAt(j)).getValue(0) * ((Point3D)intersectingPoints.elementAt(i)).getValue(0) + 
                                       ((Point3D)allplanes.elementAt(j)).getValue(1) * ((Point3D)intersectingPoints.elementAt(i)).getValue(1) + 
                                       ((Point3D)allplanes.elementAt(j)).getValue(2) * ((Point3D)intersectingPoints.elementAt(i)).getValue(2);
                            // if vec(n) * vec(point)  -  plain(D) = 0
                            // point is member of this plain
                            if (Math.abs(tmp1 - ((Point3D)allplanes.elementAt(j)).getValue(3)) < EPSILON) {
                                    erg = new double[3];
                                    erg[0] = ((Point3D)intersectingPoints.elementAt(i)).getValue(0);
                                    erg[1] = ((Point3D)intersectingPoints.elementAt(i)).getValue(1);
                                    erg[2] = ((Point3D)intersectingPoints.elementAt(i)).getValue(2);
                                    surfacePoints[j].addElement(new Point3D(erg));
                            }
                    }
            }
            //Volume
                    ...
                    if (surfacePoints[i].size() > 2) {
                            ...
                            for (j = surfacePoints[i].size() - 2; j >= 0; j--) {
                                    for ( k = 1; k <= j; k++) {
                                            ...
                                            // 2. point = starting point + normalvector
                                            for ( l = 0; l < 3; l++) {
                                                    matDet[1][l] = ((Point3D)surfacePoints[i].elementAt(0)).getValue(l) + ((Point3D)allplanes.elementAt(i)).getValue(l);
                                            }
                                            matDet[1][3] = 1.0;
                                            // 3. point
                                            for ( l = 0; l < 3; l++) {
