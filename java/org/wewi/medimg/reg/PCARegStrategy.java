...
public class PCARegStrategy implements RegStrategy {
	...
	/**
	  ...
	  */
	public Transformation calculate(RegisterParameter param)
			...
			if (ti1.hasNext()) {
				...
				if (ti2.hasNext()) {
					...
					try {
						...
						affinityMetric.setTransformation(
							transformResult.getTransformByTissue(ft));
						System.out.println(
							" Abbildung für Tissue " + ft.toString());
						transformResult.insertFitness(
							affinityMetric.similarity());
					} catch (RegistrationException re) {
						System.out.println(
							"Abbildung fuer Feature "
								+ ft.toString()
								+ " entfernt");
						re.printStackTrace();
		...
		timer3.start();
		AffineInterpolation retVal =
			(AffineInterpolation) (weightStrategy.interpolate(transformResult));
		timer3.stop();
		timer3.print();
	 ...
	 */
	private AffineInterpolation getTransform(
		TissueIterator ti1,
		TissueIterator ti2)
		throws RegistrationException {
		AffineInterpolation trans = new AffineInterpolation();
	 ...
	 * und zurückgegeben.
	 */
	private DoubleMatrix2D getHotellingTransform(
		DoubleMatrix2D data,
		DoubleMatrix1D cog,
		...
		for (i = 0; i < p; i++) {
			for (j = 0; j < p; j++) {
				if (eigenValues1D.getQuick(i)
					== eigenValues1DSorted.getQuick(j)
					&& indexes[i] == -1) {
					indexes[i] = j;
				}
			}
		...
		//}
		for (i = 0; i < p; i++) {
			eigenVectors.viewColumn(i).assign(
				eigenVectors_us.viewColumn(indexes[i]));
		}
	 ...
	 */
	private DoubleMatrix1D getMedian(
		DoubleMatrix2D data,
		DoubleMatrix2D transform) {
		int cols = data.columns();
		int rows = data.rows();
